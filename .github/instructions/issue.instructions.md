...
Where the relevant code lives and why it behaves this way. Link files using workspace-relative paths with line numbers (e.g. `[file.ts](src/file.ts#L42)`).
...
- [ ] Verified on relevant platforms (when UI-facing)
```
...
- Skip "Steps to reproduce" only for pure editor / content / docs tasks.
- Code references: backticks for symbols, markdown links for files. Never bare file paths.

# Labels

Standard set: `bug`, `documentation`, `duplicate`, `enhancement`, `good first issue`, `help wanted`, `invalid`, `question`, `wontfix`.

- `bug`: something is broken for the user.
- `enhancement`: new behaviour or visible improvement.
- `question`: blocked on a stakeholder decision. Combine with `enhancement` when applicable.
- `documentation`: README, instructions, comments only.

# CLI

- Always prefix `gh` with `GH_PAGER=cat` (terminal opens an alternate buffer otherwise).
- Resolve the target repo from the current workspace (`gh repo view --json nameWithOwner`).
